...
func main() {
	...
	for i := 0; i < len(uniqueMispell); i++ {
		...
		nysiisQueryToken := matchr.NYSIIS(uniqueMispell[i])

		if dis <= maxEditDistance {
			for _, w := range l[:maxResults] {
				nysiisFilteredPrediction := matchr.NYSIIS(w.Word)
	...
	WriteToFile(results, "./output/results-NYSIIS.json")
	...
	performance = append(performance,
		Metrics{Precision: precision, Recall: recall, Algorithm: "Levenshtein-NYSIIS"})

	WriteToFile(performance, "./output/metrics-NYSIIS.json")
	t := time.Now()
	elapsed := t.Sub(start)
