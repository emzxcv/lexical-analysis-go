...
func main() {
	...
	fmt.Println("Beginning Levenshtein-Phonex evaluation...")
	...
	for i := 0; i < len(uniqueMispell); i++ {
		...
		nysiisQueryToken := matchr.Phonex(uniqueMispell[i])

		if dis <= maxEditDistance {
			for _, w := range l[:maxResults] {
				nysiisFilteredPrediction := matchr.Phonex(w.Word)
	...
	WriteToFile(results, "./output/results-phonex.json")
	...
	performance = append(performance,
		Metrics{Precision: precision, Recall: recall, Algorithm: "Levenshtein-phonex"})

	WriteToFile(performance, "./output/metrics-phonex.json")
	t := time.Now()
	elapsed := t.Sub(start)
