...
func main() {
	...
	fmt.Println("Beginning Levenshtein-Soundex evaluation...")
	...
	for i := 0; i < len(uniqueMispell); i++ {
		...
		nysiisQueryToken := matchr.Soundex(uniqueMispell[i])

		if dis <= maxEditDistance {
			for _, w := range l[:maxResults] {
				nysiisFilteredPrediction := matchr.Soundex(w.Word)
				if w.Distance == dis && nysiisQueryToken == nysiisFilteredPrediction {
	...
	WriteToFile(results, "./output/results-soundex.json")
	...
	performance = append(performance,
		Metrics{Precision: precision, Recall: recall, Algorithm: "Levenshtein-Soundex"})

	WriteToFile(performance, "./output/metrics-soundex.json")
	t := time.Now()
	elapsed := t.Sub(start)
